#!/usr/bin/env python3
"""Command-line entry point of the annotation harness.
...
Example (20-question smoke test: dry-run for the budget first, then the real call):
  python scripts/synth_harness/run_harness.py --config configs/synth_harness_deepseek.json \
    ...
    --tasks sentence_score --min-sentences 3 --max-sentences 40 --limit 20 --output-dir data/segment_scores/smoke --dry-run
...
try:
    from .pipeline import (build_providers, build_tasks, estimate_cost, load_items, load_reference_stems, run,
...
def parse_args(argv=None):
    ...
    parser.add_argument("--input", required=True, help="messages-format rows with official rationales (see data/README.md)")
    parser.add_argument("--exclude-stems", nargs="*", default=[], help="evaluation-set files: their stems are never sent to the model")
    parser.add_argument("--allow-no-exclude", action="store_true", help="allow running without reference files (local tests only)")
    parser.add_argument("--tasks", default="sentence_score")
    parser.add_argument("--limit", type=int, default=0)
    parser.add_argument("--keep-shuffled", action="store_true", help="by default one row per stem (shuffled copies skipped)")
    parser.add_argument("--min-sentences", type=int, default=0, help="keep only items whose rationale has at least this many segments (0 = no limit)")
...
def main(argv=None):
    ...
    if not args.exclude_stems and not args.allow_no_exclude:
        ...
        return 2
    exclude_hashes, _ = load_reference_stems(args.exclude_stems) if args.exclude_stems else (set(), [])
    items, load_counts = load_items(args.input)
    ...
    selected, select_counts = select_items(items, args.limit, args.seed, frozenset(exclude_hashes), only_original=not args.keep_shuffled)
    ...
    records, exports, funnels = run(cfg, selected, tasks, providers, budget, cache, args.workers, frozenset(exclude_hashes))
    manifest = write_outputs(args.output_dir, cfg, cfg_text, [args.input] + list(args.exclude_stems),
